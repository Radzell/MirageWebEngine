 ...
 */
inline void match(const Mat& m_grayImg, const vector<KeyPoint> &trainKeys, const Mat &trainDes, const vector<vector<KeyPoint> > &queryKeys, const vector<Mat> &queryDes, vector<pair<float, int> > &result) {
	int runs = 0;
